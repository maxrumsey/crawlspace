 ...
 */
module.exports = async (scheduled, opts) => {
	// Logging
	if (opts.level >= 2) console.log(`${buildTimeStamp()} Crawl beginning. Crawling ${scheduled.length} page(s).`)
	...
	for (var i = scheduled.length - 1; i >= 0; i--) {
		...
		// Logging
		if (opts.level == 3) console.log(`Crawling page: ${title}`);
		...
		for (var j = a.length - 1; j >= 0; j--) {
			...
			if (url == '#') continue;
			else if (opts.prefix && url.startsWith('http') && !url.startsWith(opts.prefix)) continue;
			...
			// Logging
			if (opts.level == 3) console.log('Pushing URL: ' + push.url)
	...
	// Logging once done
	if (opts.level == 3) console.log(obj)
	if (opts.level >= 2) console.log(`${buildTimeStamp()} Crawl done. Returned ${obj.indexeds.length} indexed record(s).`)
	if (opts.level >= 2) console.log(`${buildTimeStamp()} Added ${obj.scheduleds.length} record(s) to the queue.`)
	return obj
}
